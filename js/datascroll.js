...
// @param (boxname) 选项存放的容器
// @param (options.ratio) 滑动西数
// @param (options.defaultIndex) 默认第几个
// @param (options.datainput)输入框
// @param (options.initCallback)初始化完回调
// @param (options.endCallback)滚动完回调
// @param (options.sureCallback)确认时回调
	...
	pickScroll = function(boxname,options){
		var that = this;
			...
			that.options = {
				...
				dataInput : document.querySelector(options.datainput), //输入框
	...
	pickScroll.prototype = {
		...
		_boxClick: function(){
			...
			that.options.dataInput.onclick = function(e){
				e.stopPropagation();
				if(document.querySelector(".input_data_clickon")){
					document.querySelector(".input_data_clickon").classList.remove("input_data_clickon");
				}
				this.classList.add("input_data_clickon");
